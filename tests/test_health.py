...
def test_unauthorized_missing_bearer():
    ...
    with app.test_client() as client:
        # 没有Bearer前缀
        resp = client.get('/sync/all', headers={'Authorization': access_token})
        assert resp.status_code == 401

def test_unauthorized_empty_auth_header():
    """测试空认证头"""
    with app.test_client() as client:
        resp = client.get('/sync/all', headers={'Authorization': ''})
        assert resp.status_code == 401

def test_unauthorized_wrong_auth_scheme():
    """测试错误的认证方案"""
    with app.test_client() as client:
        resp = client.get('/sync/all', headers={'Authorization': 'Basic sometoken'})
        assert resp.status_code == 401
